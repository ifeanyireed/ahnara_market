...
export default function CheckoutPage() {
  ...
  const handlePlaceOrder = () => {
    ...
    // Create a new order object to save in history
    ...
    // Save to localStorage buyer orders
    ...
    alert(`Order ${orderId} successfully placed! Redirecting to your Buyer Portal.`);
    router.push("/buyer");
  };
  ...
  return (
    <div className="min-h-screen bg-[#E8EFF4] text-[#0D090C] font-sans flex flex-col text-left">
      <header className="px-8 py-5 border-b border-slate-200/50 bg-white/40 backdrop-blur-md flex items-center justify-between">
        <Link href="/" className="flex items-center gap-1 text-xs font-black text-slate-700 hover:text-slate-950 transition-all">
          <IconArrowLeft className="w-4 h-4" />
          Back to Store
        </Link>
        <span className="font-extrabold text-sm tracking-tight text-slate-900 uppercase">Fulfillment Checkout</span>
        <div className="w-16" />
      </header>

      <main className="max-w-4xl w-full mx-auto px-6 py-8 grid grid-cols-1 md:grid-cols-3 gap-6">
        
        {/* Left Columns: Shipment & Payments */}
